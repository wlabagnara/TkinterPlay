"""
GUI application to download a webpage into a textbox.
reference: https://www.pythontutorial.net/tkinter/tkinter-thread/
...
class AsyncDownload(Thread): # thread task to perform download operation
...
class App(tk.Tk): # application main window derived from tk GUI
    def __init__(self):
        super().__init__()
    ...
    def monitor(self, thread):
        if thread.is_alive():
            self.after(200, lambda: self.monitor(thread)) # check every 100ms
        else:
